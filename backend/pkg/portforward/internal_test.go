...
func TestGetPortForwardList_ClusterPrefixCollision(t *testing.T) {
	p1 := portForward{ID: "id1", Cluster: "foo"}
	p2 := portForward{ID: "id2", Cluster: "foobar"}
	...
	pfList := getPortForwardList(cache, "foo")
	assert.ElementsMatch(t, []portForward{p1}, pfList)

	pfList = getPortForwardList(cache, "foobar")
	assert.ElementsMatch(t, []portForward{p2}, pfList)
}
...
func TestGetPortForwardList_UserIDCollision(t *testing.T) {
	...
	userPF := portForward{ID: "pf-user", cacheCluster: portforwardClusterKey("foo", "bar"), Cluster: "foo", Status: RUNNING}
